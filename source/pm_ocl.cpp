...
static double profileKernel(cl::Kernel &kernel, cl::CommandQueue &queue, cl::NDRange work_group, cl::Buffer &bits, int offset_x, int offset_y, img_data *idata, proc_data *pdata)
{
    cl::Event event;
...
void pm_parallel(img_data *idata, proc_data *pdata, cl_data *cdata)
{
    ...
    if(cdata->bitcode) {
        ...
        auto binaries = cl::Program::Binaries {std::make_pair<const void*, ::size_t>(cdata->filename.c_str(), cdata->filename.length())};
    ...
    if(global_size < idata->size * sizeof(uint)) {
        std::stringstream ss;
        ss << "Image size is too large, max available memory size for device " << cdata->deviceId << " is " << global_size << std::endl;
